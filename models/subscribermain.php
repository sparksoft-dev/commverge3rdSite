...
class Subscribermain extends CI_Model
{
	...
	public $DEFAULT_COLUMNS = [
		'ACCOUNT TYPE', 'REALM', 'USERNAME', 'STATUS', 'ADDITIONAL SERVICE 1', 'ADDITIONAL SERVICE 2',
		'ORDER NUMBER', 'SERVICE NUMBER', 'CUSTOMER NAME', 'IP ADDRESS', 'NET ADDRESS', 'REMARKS'
	];
	...
	public function create($s, $username = null, $password = null, $host = null, $port = null, $schema = null)
	{
		try {
			...
			$sql = "insert into TBLCUSTOMER (CUI, USER_IDENTITY, USERNAME, BANDWIDTH, CUSTOMERSTATUS, PASSWORD, CUSTOMERREPLYITEM, CREATEDATE, LASTMODIFIEDDATE, RBACCOUNTPLAN, RADIUSPOLICY, " .
				...
				"(:cui, :useridentity, :username, null, :customerstatus, :password, " . ($s['CUSTOMERREPLYITEM'] == '' || is_null($s['CUSTOMERREPLYITEM']) ? "null" : ":customerreplyitem") . ", " .
				"TO_TIMESTAMP(:now, 'RR-MM-DD HH24:MI:SS'), null, :radiuspolicy, :radiuspolicy, :rbcustomername, :rbcreatedby, null, " . (is_null($s['RBADDITIONALSERVICE4']) ? "null" : ":rbadditionalservice4") . ", null, " . (is_null($s['RBADDITIONALSERVICE2']) ? "null" : ":rbadditionalservice2") . ", " . (is_null($s['RBADDITIONALSERVICE1']) ? "null" : ":rbadditionalservice1") . ", " .
				"null, null, " . ($s['CUSTOMERSTATUS'] == 'Active' ? "TO_TIMESTAMP(:now, 'RR-MM-DD HH24:MI:SS')" : "null") . ", " . ($s['CUSTOMERSTATUS'] == 'Active' ? ":rbactivatedby" : "null") . ", :rbaccountstatus, null, :rbsvccode, :customertype, :rbservicenumber, null, null, 1, " .
			...
			if ($s['CUSTOMERREPLYITEM'] == '' || is_null($s['CUSTOMERREPLYITEM'])) { } else {
			...
			if ($result === false) {
				...
				try {
					$now = date('Y-m-d H:i:s', time());
					log_message('info', 'connection: ' . $host . ':' . $port . '/' . $schema . ', ' . $username . '|' . $password);
					$conn2 = oci_connect($username, $password, $host . ':' . $port . '/' . $schema);
					if (!$conn2) {
						return false;
					}
					$sql = "insert into TBLCUSTOMER (CUI, USER_IDENTITY, USERNAME, BANDWIDTH, CUSTOMERSTATUS, PASSWORD, CUSTOMERREPLYITEM, CREATEDATE, LASTMODIFIEDDATE, RBACCOUNTPLAN, RADIUSPOLICY, " .
					...
					if ($result === false) {
						...
						try {
							$now = date('Y-m-d H:i:s', time());
							log_message('info', 'connection: ' . $host . ':' . $port . '/' . $schema . ', ' . $username . '|' . $password);
							$conn3 = oci_connect($username, $password, $host . ':' . $port . '/' . $schema);
							if (!$conn3) {
								return false;
							}
							$sql = "insert into TBLCUSTOMER (CUI, USER_IDENTITY, USERNAME, BANDWIDTH, CUSTOMERSTATUS, PASSWORD, CUSTOMERREPLYITEM, CREATEDATE, LASTMODIFIEDDATE, RBACCOUNTPLAN, RADIUSPOLICY, " .
		...
		try {
			...
			$sql = "update TBLCUSTOMER set BANDWIDTH = " . (is_null($s['BANDWIDTH']) ? "null" : ":bandwidth") . ", CUSTOMERSTATUS = :customerstatus, PASSWORD = :password, " .
				...
				"RBCHANGESTATUSDATE = " . (is_null($s['RBCHANGESTATUSDATE']) ?
					"null" : (strpos($s['RBCHANGESTATUSDATE'], 'AM') === false && strpos($s['RBCHANGESTATUSDATE'], 'PM') === false ?
						"TO_TIMESTAMP(:rbchangestatusdate, 'RR-MM-DD HH24.MI.SS')" : "TO_TIMESTAMP(:rbchangestatusdate, 'DD-MON-RR HH.MI.SS.FF " . substr($s['RBCHANGESTATUSDATE'], -2) . "')")) . ", " .
				"RBCHANGESTATUSBY = " . (is_null($s['RBCHANGESTATUSBY']) ? "null" : ":rbchangestatusby") . ", " .
				"RBACTIVATEDDATE = " . (is_null($s['RBACTIVATEDDATE']) ?
					"null" : (strpos($s['RBACTIVATEDDATE'], 'AM') === false && strpos($s['RBACTIVATEDDATE'], 'PM') === false ?
						"TO_TIMESTAMP(:rbactivateddate, 'RR-MM-DD HH24.MI.SS')" : "TO_TIMESTAMP(:rbactivateddate, 'DD-MON-RR HH.MI.SS.FF " . substr($s['RBACTIVATEDDATE'], -2) . "')")) . ", " .
	...
	public function changeSubscriberPassword($uusername, $realm, $upassword, $username = null, $password = null, $host = null, $port = null, $schema = null)
	{
		...
		*/
		...
		/* 09/26/19 Added tri-nested catch */
		try {
			...
			if ($result === false) {
				...
				try {
					...
					oci_bind_by_name($compiled, ':cn', strval($cn));
					$result = oci_execute($compiled, OCI_NO_AUTO_COMMIT);
					/* ++ nested try catch  if (result) roll back*/
					if ($result === false) {
						oci_rollback($conn2);
						$error = oci_error($compiled);
						return array('result' => false, 'sql' => $sql, 'error' => json_encode($error));
			...
			// Add later return $result ? true : false;
		} catch (Exception $e) {
			return array('result' => false, 'sql' => $sql, 'error' => json_encode($e));
		}
	}
	...
	public function unassignIPv6IPAndNetAddress($cn, $ipv6 = false, $ip = false, $net = false, $replyItem = null, $username = null, $password = null, $host = null, $port = null, $schema = null)
	{
		...
		$sql = "update TBLCUSTOMER set LASTMODIFIEDDATE = TO_TIMESTAMP(:now, 'RR-MM-DD HH24:MI:SS')" . ($ipv6 ? ", RBADDITIONALSERVICE4 = null" : "") . ($ip ? ", RBIPADDRESS = null" : "") . ($net ? ", RBMULTISTATIC = null" : "") .
	...
	public function searchSubscriber($column, $value, $exact, $wildcardLocation, $portal, $realm, $allowedRealms)
	{ //wildcardLocation = 'before'/'after'/'both'
	...
	public function isValid($subscriber)
	{
		$errors = array();
		if (
			$subscriber[$this->COLUMN_USERIDENTITY] == null || trim($subscriber[$this->COLUMN_USERIDENTITY]) == '' || $subscriber[$this->COLUMN_USERNAME] == null ||
	...
	public function rowDataToSubscriberArrayV2($data, $realm, $sysuser, $action)
	{
		...
		$subscriber[$this->COLUMN_CUSTOMERREPLYITEM] = $this->generateCustomerReplyItemValue(
			$subscriber[$this->COLUMN_RBADDITIONALSERVICE4],
			$subscriber[$this->COLUMN_RBIPADDRESS],
			$subscriber[$this->COLUMN_RBMULTISTATIC]
		);
		return $subscriber;
	}
	...
	public function npmCreateXML(
		...
		if (!is_null($ipaddress)) {
			$xmlStr = $xmlStr .
				'<CircuitAttributes>' .
				'<CircuitAttribute>' .
				'<SessionFilter><Session-Name>' . $cn . '</Session-Name></SessionFilter>' . (is_null($ipaddress) ? '' : '<Static-IP-Address>' . $ipaddress . '</Static-IP-Address>') . (is_null($netaddress) ? '' : '<Framed-Route>' . $netaddress . ' ' . $ipaddress . '</Framed-Route>') .
				'</CircuitAttribute>' .
				'</CircuitAttributes>';
		}
	...
	public function npmUpdateXML(
		...
		if (!is_null($ipaddress)) {
			$xmlStr = $xmlStr .
				'<CircuitAttributes>' .
				'<CircuitAttribute>' .
				'<SessionFilter><Session-Name>' . $cn . '</Session-Name></SessionFilter>' . (is_null($ipaddress) ? '' : '<Static-IP-Address>' . $ipaddress . '</Static-IP-Address>') . (is_null($netaddress) ? '' : '<Framed-Route>' . $netaddress . (count($netAddrParts) == 1 ? ' ' . $ipaddress : '') . '</Framed-Route>') .
				'</CircuitAttribute>' .
				'</CircuitAttributes>';
		}
		$xmlStr = $xmlStr .
