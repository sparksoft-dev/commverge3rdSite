...
require_once '/var/www/html/executables/api/subscriberapiwsdlresultcodes.php';
require_once '/var/www/html/executables/api/authentication.php';
require_once '/var/www/html/executables/api/rm.php';
...
function wsCreateAccount($param) {
	...
	// Added AAA database 2 connection 8/8/2019
	/****************************************
	* AAA database 2 connection
	****************************************/
	$aaaDb2 = oci_connect($config['db2SessionUsername'], $config['db2SessionPassword'], $config['db2SessionUrl']);
	if ($aaaDb2 === false) {
		writeToFile( $apiAccessLogDir.$functionName.$logFile.".log", '----- '.$functionName.' respopnse to '.$clientIp.': ['.F_ORACLE_DB2_CONNECT_ERROR.'] No connection to AAA database 2. {username: '.$username.'}');
		$returnObj['responseCode'] = F_ORACLE_DB2_CONNECT_ERROR;
		$returnObj['replyMessage'] = 'AAA database 2 error occurred';
		return $returnObj;
	}

	// Added AAA3 database connection 8/13/2019
	/****************************************
	* AAA3 database connection
	****************************************/
	$aaaConn4 = oci_connect($config['quaternarySessionUsername'], $config['quaternarySessionPassword'], $config['quaternarySessionUrl']);
	if ($aaaConn4 === false) {
		writeToFile( $apiAccessLogDir.$functionName.$logFile.".log", '----- '.$functionName.' respopnse to '.$clientIp.': ['.F_ORACLE_DB3_CONNECT_ERROR.'] No connection to AAA3 database. {username: '.$username.'}');
		$returnObj['responseCode'] = F_ORACLE_DB3_CONNECT_ERROR;
	...
	if ($plan == '') {
		...
		return $returnObj;
		EDITED 05162019  */
		$plan = str_replace('~', '-', $accountOld['data']['RBACCOUNTPLAN']);
	}
		...
		// other response codes returned by original insert attempt
			...
			if ($found1 && $found2) {
				...
				// rm update for special case failed
					...
					// if response is alternate id already exists; return 306 instead
					if(strpos($rmUpdate['responseMessage'], "Reason: Alternate Id:" ) !== false ) {
						writeToFile( $apiAccessLogDir.$functionName.$logFile.".log", '----- '.$functionName.' response to '.$clientIp.': ['.F_USERNAME_ALREADY_USED.'] Error at RM: '.
						...
						/*
							...
							// 	}
							// }
							// $returnObj['responseCode'] = 306;
							// $returnObj['replyMessage'] = "Alternate ID already exists";
						*/

						// [AJB] Added 030618 - START --------
						// deleting/purging all variants of #LX (X -> 1 to 3), including no #LX
						writeToFile( $apiAccessLogDir.$functionName.$logFile.".log", "Alternate ID exists, deleting and purging..");
						for ($i = 0; $i < 4; $i++) {
							if ($i == 0) {
								$useThisUsername = $username;
								$rmDelete = Rm::wsDeleteSubscriberProfile($rmApiClient, $useThisUsername);
			...
			// accounts for other response codes returned by original insert
				...
				if(strpos($rmInsert['responseMessage'], "Reason: Alternate Id:" ) !== false ) {
					writeToFile( $apiAccessLogDir.$functionName.$logFile.".log", '----- '.$functionName.' response to '.$clientIp.': ['.F_USERNAME_ALREADY_USED.'] Error at RM: '.
						$rmInsert['responseCode'].'|'.$rmInsert['responseMessage']);
					$lrq = logRequest($mysqlConn, $clientIp, $functionName, $param, F_USERNAME_ALREADY_USED);
					if (!$lrq['result']) {
				...
				//edited 05202019 return $returnObj;
			}
		}
	} else {
		writeToFile( $apiAccessLogDir.$functionName.$logFile.".log", '----- '.$functionName.' respopnse to '.$clientIp.': [ SKIPPING RM, RMREVERTENABLE IS SET TO FALSE ]');
		return $returnObj;
	}
}
function wsModifyAccount($param) {
	 ...
	 **************************************************/
	try {
		$params = array('location' => 'http://'.$config['primaryDeleteSessionUrl']);
		$deleteSessionClient = new SoapClient('http://'.$config['primaryDeleteSessionUrl'], $params);
	} catch (Exception $e) {
		$error = json_encode($e);
		$start = strpos($error, '"faultstring":"') + strlen('"faultstring":"');
		$end = strpos($error, '","faultcode"');
	...
	if ($config['useSecondary']) {
		try {
			$params = array('location' => 'http://'.$config['secondaryDeleteSessionUrl']);
			$deleteSessionClientSecondary = new SoapClient('http://'.$config['secondaryDeleteSessionUrl'], $params);
		} catch (Exception $e) {
			$error = json_encode($e);
			$start = strpos($error, '"faultstring":"') + strlen('"faultstring":"');
			$end = strpos($error, '","faultcode"');
	...
	if(RMREVERTENABLE){
		...
		// account not found in rm, will try to insert
			...
			// insert failed
				...
				writeToFile( $apiAccessLogDir.$functionName.$logFile.".log", "Returned ". $rmInsert['responseCode'].'|'.$rmInsert['responseMessage']);
				$lrq = logRequest($mysqlConn, $clientIp, $functionName, $param, F_GENERIC_RM_ERROR);
				if (!$lrq['result']) {
					writeToFile( $apiAccessLogDir.$functionName.$logFile.".log", 'Unable to log request');
				}
...
function wsChangeUsername($param) {
	...
	// [AJB 031618] START: Added checking of username for format
	$isUsernameValid = true;
	// check for uppercase characters
	$usernameHasUppercase = preg_match('/[A-Z]{1}/', $newUsername);
	if ($usernameHasUppercase) {
		$isUsernameValid = false;
	}
	// check for non-alphanumeric characters
	$usernameHasSpecialChars = preg_match('/[^a-zA-Z0-9.@_-]/', $newUsername);
	if ($usernameHasSpecialChars) {
		$isUsernameValid = false;
	}

	if (!$isUsernameValid) {
		writeToFile( $apiAccessLogDir.$functionName.$logFile.".log", '----- '.$functionName.' response to '.$clientIp.': ['.F_INVALID_USERNAME_HAS_DISALLOWED_CHARACTERS.'] Invalid new username. {username: '.$newUsername.'}');
		$lrq = logRequest($mysqlConn, $clientIp, $functionName, $param, F_INVALID_USERNAME_HAS_DISALLOWED_CHARACTERS);
		if (!$lrq['result']) {
			writeToFile( $apiAccessLogDir.$functionName.$logFile.".log", 'Unable to log request');
		}
	...
	// check for username length
	if (strlen($newUsername) > Aaa::$USERNAME_MAX_LENGTH) {
		writeToFile( $apiAccessLogDir.$functionName.$logFile.".log", '----- '.$functionName.' response to '.$clientIp.': ['.F_CHARACTER_LIMIT_EXCEEDED.'] Maximum characters exceeded: username. {username: '.$newUsername.'}');
		$lrq = logRequest($mysqlConn, $clientIp, $functionName, $param, F_CHARACTER_LIMIT_EXCEEDED);
		if (!$lrq['result']) {
			writeToFile( $apiAccessLogDir.$functionName.$logFile.".log", 'Unable to log request');
		}
	...
	// check if a realm is included
	$planParts = explode('@', $newUsername);
	if (!isset($planParts[1])) {
		writeToFile( $apiAccessLogDir.$functionName.$logFile.".log", '----- '.$functionName.' response to '.$clientIp.': ['.F_INVALID_USERNAME_NO_REALM.'] Username has no realm. {username: '.$newUsername.'}');
		$lrq = logRequest($mysqlConn, $clientIp, $functionName, $param, F_INVALID_USERNAME_NO_REALM);
		if (!$lrq['result']) {
			writeToFile( $apiAccessLogDir.$functionName.$logFile.".log", 'Unable to log request');
		}
	...
	$realmFound = in_array($planParts[1], $realms);
	if (!$realmFound) {
		writeToFile( $apiAccessLogDir.$functionName.$logFile.".log", '----- '.$functionName.' response to '.$clientIp.': ['.F_INVALID_USERNAME_UNLISTED_REALM.'] Username has invalid realm. {username: '.$newUsername.'}');
		$lrq = logRequest($mysqlConn, $clientIp, $functionName, $param, F_INVALID_USERNAME_UNLISTED_REALM);
		if (!$lrq['result']) {
			writeToFile( $apiAccessLogDir.$functionName.$logFile.".log", 'Unable to log request');
		}
	...
	// [AJB 031618] END: Added checking of username for format
...
function wsAddIPAddress($param) {
	 ...
	 * RM client
	 **************************************************/
	/*EDITED 05162019 Assume RMREVERTENABLE == FALSE
	try {
		$rmApiClient = new SoapClient('http://'.$config['primaryRmUrl'])	;
	} catch (Exception $e) {
		$error = json_encode($e);
...
function wsSearchAccount($param) {
	...
	*/
	...
	$returnObj['customerType'] = '';
	$returnObj['redirected'] = '';
...
function wsAddVOD($param) {
	...
	writeToFile( $apiAccessLogDir.$functionName.$logFile.".log", '----- '.$functionName.' response to '.$clientIp.' Passing params: '.$newVodFormat.' '.json_encode($param) );
...
function generateConnectionUrls() {
	...
	// added for db2 8/8/2019
	$sessionHost3 = explode('=', $lines[81]);
	$sessionPort3= explode('=', $lines[82]);
	$sessionSchema3 = explode('=', $lines[83]);
	$sessionUsername3 = explode('=', $lines[84]);
	$sessionPassword3 = explode('=', $lines[85]);
	// added for db3 8/13/2019
	$sessionHost4 = explode('=', $lines[86]);
	$sessionPort4= explode('=', $lines[87]);
	$sessionSchema4 = explode('=', $lines[88]);
	$sessionUsername4 = explode('=', $lines[89]);
	$sessionPassword4 = explode('=', $lines[90]);
	...
	$configObj = array(
		...
		// added for db2 8/8/2019
		'db2SessionUrl' => $sessionHost3[1].':'.$sessionPort3[1].'/'.$sessionSchema3[1],
		'db2SessionUsername' => $sessionUsername3[1],
		'db2SessionPassword' => $sessionPassword3[1],
		// added for db3 8/13/2019
		'db3SessionUrl' => $sessionHost4[1].':'.$sessionPort4[1].'/'.$sessionSchema4[1],
		'db3SessionUsername' => $sessionUsername4[1],
		'db3SessionPassword' => $sessionPassword4[1],
	return $configObj;
}
