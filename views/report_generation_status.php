...
<body style="padding-right:20px; padding-bottom:20px;">
	<div align="left">
		<h3 class="style1">Report Generation: Status</h3>
	</div>
	...
	<div align="left">
		...
		<form name="frmMain" action="<?php echo base_url('reports/generateSubscriberStatusReport'); ?>" method="post">
			<table cellspacing="0" cellpadding="3" border="0">
				<tbody>
					<tr>
						<td class="xsmallFontWHTBG" align="middle">Account Status</td>
						<td class="xsmallFontWHTBG" align="middle">
							<select size="1" name="status" class="xsmallFontWHTBG">
						...
						<td class="xsmallFontWHTBG" align="middle">
							...
							<?php
							//if (isset($count) && ($count >= 200000 && $count < 400000)) {
							?>
							<input class="button2" type="submit" name="submit" value="extract (2)" />
							<?php
							//} else if (isset($count) && $count >= 400000) {
							?>
							<input class="button2" type="submit" name="submit" value="extract (2)" />
							<input class="button2" type="submit" name="submit" value="extract (3)" />
							<?php
							//}
							?>
							-->
		...
		<table width="100%">
			<tr>
				<td colspan="10">
					...
					if ($pageStart != 0) {
						$j = $pageStart - 1;
						echo '<a href="'.base_url("reports/generateSubscriberStatusReport/1/count/".$status."/".($j * $max)."/".$max).'">&lt;&lt;</a>';
						echo ' |';
					}
					for ($j = $pageStart; $j < $pageEnd; $j++) {
						echo '<a href="'.base_url("reports/generateSubscriberStatusReport/1/count/".$status."/".($j * $max)."/".$max).'">'.($j + 1).'</a>';
						echo ' | ';
					}
					if ($pageEnd < $pages) {
						$j = $pageEnd;
	...
	<script type="text/javascript">
		...
		$('select[name="max"]').on('change', function (event) {
			 var _this = $(this),
				max = _this.val(),
				location = '<?php echo base_url("reports/generateSubscriberStatusReport/1/count/".$status."/0"); ?>';
		...
		$(document).ready(function () {
			$('#extractBtn').on('click', function (evt) {
				...
				$.ajax({
					url: '<?php echo base_url("reports/ajaxGenerateSubscriberStatusReport"); ?>',
					type: 'POST',
					data: {
						'status': status
					},
					timeout: 1800000, //(milliseconds) 30 minutes
					success: function (resp) {
						console.log('received from server: encoded \$data object');
						console.log(resp);
					...
					error: function (request, textStatus, errorThrown) {
						console.log('error at ajaxGenerateSubscriberStatusReport:');
						console.log(request);
