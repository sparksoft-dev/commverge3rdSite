...
<body style="padding-right:20px; padding-bottom:20px;">
	<div align="left">
		<h3 class="style1">Report Generation: Static IP Address</h3>
	</div>
	...
	<div align="left">
		...
		<form name="frmMain" action="<?php echo base_url('reports/generateSubscriberIpReport'); ?>" method="post">
			<table cellspacing="0" cellpadding="3" border="0">
				<tbody>
					<tr>
						<td class="xsmallFontWHTBG" align="middle">Subscribers with Static IP Address</td>
		...
		<table width="100%">
			<tr>
				<td colspan="10">
					...
					if ($pageStart != 0) {
						$j = $pageStart - 1;
						echo '<a href="'.base_url("reports/generateSubscriberIpReport/1/count/".($j * $max)."/".$max).'">&lt;&lt;</a>';
						echo ' |';
					}
					for ($j = $pageStart; $j < $pageEnd; $j++) {
						echo '<a href="'.base_url("reports/generateSubscriberIpReport/1/count/".($j * $max)."/".$max).'">'.($j + 1).'</a>';
						echo ' | ';
					}
					if ($pageEnd < $pages) {
						$j = $pageEnd;
	...
	<script type="text/javascript">
		...
		$('select[name="max"]').on('change', function (event) {
			 var _this = $(this),
				max = _this.val(),
				location = '<?php echo base_url("reports/generateSubscriberIpReport/1/count/0"); ?>';
		...
		$(document).ready(function () {
			$('#extractBtn').on('click', function (evt) {
				...
				$.ajax({
					url: '<?php echo base_url("reports/ajaxGenerateSubscriberIpReport"); ?>',
					type: 'POST',
