...
<body style="padding-right:20px; padding-bottom:20px;">
	<div>
		<h3 class="style1">Locations: Modify</h3>
	</div>
	...
	<div>
		<form name="editForm" action="<?php echo base_url('main/processLocationModification'); ?>" method="post">
			<table cellpadding="2">
				<tr>
				...
				<tr>
					...
					<td class="smallFontWHTBG" align="left">
						<input type="text" name="nasDescription" id="nasDescription" 
				...
				<tr>
					...
					<td class="smallFontWHTBG" align="left">
						<input type="text" name="rmDescription" id="rmDescription" 
							value="<?php echo isset($rmDescription) && !is_null($rmDescription) ? $rmDescription : ''; ?>" />
					</td>
				</tr>
				<tr>
					<td align="right" colspan="2">
						<input type="hidden" name="locationId" id="locationId" value="<?php echo isset($locationId) && !is_null($locationId) ? $locationId : ''; ?>" />
		...
		$(document).ready(function () {
			$('form[name="editForm"]').on('submit', function (event) {
				if ($('#location').val().trim() == '') {
					$('span.errorMsg').empty().append('Please enter new NAS tag.');
					$('#location').focus();
					return false;
				}
				if ($('#nasName').val().trim() == '') {
					$('span.errorMsg').empty().append('Please enter new NAS name.');
					$('#nasName').focus();
					return false;
				}
				if ($('#nasIp').val().trim() == '') {
					$('span.errorMsg').empty().append('Please enter new NAS IP.');
					$('#nasIp').focus();
					return false;
				}
				if ($('#nasCode').val().trim() == '') {
					$('span.errorMsg').empty().append('Please enter new NAS code.');
					$('#nasCode').focus();
					return false;
				}
				if ($('#rmLocation').val().trim() == '') {
					$('span.errorMsg').empty().append('Please enter new RM location.');
					$('#rmLocation').focus();
					return false;
				}
			});
