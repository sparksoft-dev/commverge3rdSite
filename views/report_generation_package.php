...
<body style="padding-right:20px; padding-bottom:20px;">
	<div align="left">
		<h3 class="style1">Report Generation: Package</h3>
	</div>
		...
		<table>
			<tr>
				<td align="left" class="smallFont" colspan="2">
					<span class="notificationMsg">Info as of <?php echo $yesterday; ?></strong></span>
				</td>
			</tr>
		</table>
		<form name="frmMain" action="<?php echo base_url('reports/generateSubscriberPackageReport'); ?>" method="post">
			<table cellspacing="0" cellpadding="3" border="0">
				<tbody>
					<tr>
						<td class="xsmallFontWHTBG" align="middle">Service</td>
						<td class="xsmallFontWHTBG" align="middle">
							<select size="1" name="service" class="xsmallFontWHTBG">
								<?php include 'select_options_services.php'; ?>
							</select>
						</td>
						<td class="xsmallFontWHTBG" align="middle">
		...
		<table width="100%">
			<tr>
				<td colspan="10">
					...
					if ($pageStart != 0) {
						$j = $pageStart - 1;
						echo '<a href="'.base_url("reports/generateSubscriberPackageReport/1/count/".str_replace(' ', '%%', $service)."/".($j * $max)."/".$max).'">&lt;&lt;</a>';
						echo ' |';
					}
					for ($j = $pageStart; $j < $pageEnd; $j++) {
						echo '<a href="'.base_url("reports/generateSubscriberPackageReport/1/count/".str_replace(' ', '%%', $service)."/".($j * $max)."/".$max).'">'.($j + 1).'</a>';
						echo ' | ';
					}
					if ($pageEnd < $pages) {
						$j = $pageEnd;
	...
	<script language="javascript">
	   <?php
		...
		$('select[name="max"]').on('change', function (event) {
			 var _this = $(this),
				max = _this.val(),
				location = '<?php echo base_url("reports/generateSubscriberPackageReport/1/count/".str_replace(' ', '%%', $service)."/0"); ?>';
		...
		$(document).ready(function () {
			$('#extractBtn').on('click', function (evt) {
				...
				$.ajax({
					url: '<?php echo base_url("reports/ajaxGenerateSubscriberPackageReport"); ?>',
					type: 'POST',
					data: {
						'service': service
					},
