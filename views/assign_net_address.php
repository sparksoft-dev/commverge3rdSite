...
<body>
				...
				<form name="assignnet">
					<table cellspacing="1" cellpadding="4" width="100%" align="center" style="background-color:#ffffff;" border="0">
						<tbody>
							...
							<tr>
								<td class="regular" align="right">Subnet:</td>
								<td>
									<select id="ipsubnet" name="popsubnet">
										<option value=""></option>
										<?php
										$subnetOptionCount = count($subnetOptions);
										for ($i = 0; $i < $subnetOptionCount; $i++) {
											$selected = false;
											if (isset($pickSubnet) && !is_null($pickSubnet) && $pickSubnet != '-') {
												$selected = intval($subnetOptions[$i]) == $pickSubnet;
											}
											echo '<option value="'.$subnetOptions[$i].'"'.($selected ? ' selected' : '').'>'.$subnetOptions[$i].'</option>';
										}
										?>
									</select>
								</td>
							...
							<tr>
								...
								<td>
									<select id="ipid" name="popipaddress">
										<?php
										if (isset($netaddresses) && !is_null($netaddresses) && $pickSubnet != '') {
											if ($location != '') {
	...
	<script type="text/javascript">
		var netLocationDropdown = $('select[name="iplocation"]'),
			...
			subnetDropdown = $('select[name="popsubnet"]'),
		...
		$(document).ready(function () {
			netLocationDropdown.on('change', function (event) {
				var _this = $(this),
					...
					location = _this.val(),
					cabinet = '',
					subnetValue = subnetDropdown.val(),
					url = '<?php echo base_url("main/showAssignNetForm"); ?>';
				username = username == '' ? '-' : username;
				realm = realm == '' ? '-' : realm;
				location = location == '' ? '-' : location;
				cabinet = location == '-' ? '-' : cabinetDropdown.find('option[location="' + location + '"]').first().text();
				if (subnetValue != '' && location != '') {
					url = url + '/' + username + '/' + realm + '/' + location.replace(/ /g, '~') + '/' + cabinet.replace(/ /g, '~') + '/-/-/-/' + subnetValue;
			...
			subnetDropdown.on('change', function (event) {
				var _this = $(this),
					subnetValue = _this.val(),
					username = $('#username').text().trim(),
					realm = $('#realm').text().trim(),
					location = netLocationDropdown.val(),
					cabinet = '',
					url = '<?php echo base_url("main/showAssignNetForm"); ?>';
				username = username == '' ? '-' : username;
				realm = realm == '' ? '-' : realm;
				location = location == '' ? '-' : location;
				cabinet = location == '-' ? '-' : cabinetDropdown.find('option[location="' + location + '"]').first().text();
				if ((subnetValue != '' || subnetValue != '-') && (location != '-' || location != '-')) {
					url = url + '/' + username + '/' + realm + '/' + location.replace(/ /g, '~') + '/' + cabinet.replace(/ /g, '~') + '/-/-/-/' + subnetValue;
					top.location.href = url;
				}
			});
						...
						if (parseInt(response.found) == 0) {
							...
							if (response.used == 'Y') {
								...
								url = url + '/' + username + '/' + realm + '/' + 
									response.location.replace(/ /g, '~') + '/' + response.cabinetForDropdown + '/' + response.ip + '/' + response.subnet + '/-/' + response.pickSubnet;
								top.location.href = url;
							}
						}
					},
					...
					success: function (resp) {
						...
						if (parseInt(response.found) == 0) {
							...
							url = url + '/' + username + '/' + realm + '/' + response.location.replace(/ /g, '~') + '/' + 
								response.cabinetForDropdown.replace(/ /g, '~') + '/-/-/' + cabinetToFind.replace(/ /g, '~') + '/' + response.pickSubnet;
							console.log(url);
							top.location.href = url;
						}
					},
