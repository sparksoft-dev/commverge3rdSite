...
<body>
	<table cellspacing="1" cellpadding="0" width="100%" align="center" style="background-color:#639ACE;" border="0">
		<tbody>
			<tr>
				<td class="regularB" align="middle" height="25"><font color="#ffffff">Assign IP Address</font></td>
			</tr>
			<tr>
				<form name="assignip">
					<table cellspacing="1" cellpadding="4" width="100%" align="center" style="background-color:#ffffff;" border="0">
						<tbody>
							...
							<tr>
								...
								<td class="regular" colspan="3">
									<?php
									$ipValue = (isset($ip) && !is_null($ip) && $ip != '-') ? $ip : '';
									?>
							...
							<tr>
								<td class="regular" align="right">IP Address:</td>
								<td>
											...
											if ($location != '') {
												...
												for ($i = 0; $i < $ipCount; $i++) {
													...
													if (isset($ip) && !is_null($ip) && $ip != '-') {
														$selected = $ipaddresses[$i]['IPADDRESS'] == $ip;
													}
													echo '<option value="'.$ipaddresses[$i]['IPADDRESS'].'"'.($selected ? ' selected' : '').'>'.
		...
		$(document).ready(function () {
			ipLocationDropdown.on('change', function (event) {
				var _this = $(this),
					...
					url = '<?php echo base_url("main/showAssignIPForm"); ?>';
				username = username == '' ? '-' : username;
				realm = realm == '' ? '-' : realm;
				location = location == '' ? '-' : location;
				cabinet = location == '-' ? '-' : cabinetDropdown.find('option[location="' + location + '"]').first().text();
				url = url + '/' + username + '/' + realm + '/' + location.replace(/ /g, '~') + '/' + gpon + '/' + cabinet.replace(/ /g, '~') + '/-/-';
				top.location.href = url;
			});
			selectIpButton.on('click', function (event) {
				var ip = ipAddressDropdown.val();
				if (ip.trim() != '') {
					window.opener.document.globeacct.ipaddress.value = ip;
			...
			findIpButton.on('click', function (event) {
				...
				realm = realm == '' ? '-' : realm;
				if (ipToFind == '') {
					findIpMsg.empty().append('Enter an ip address to find.');
					findIpField.focus();
					return false;
				}
				...
				$.ajax({
					url: '<?php echo base_url("main/findIpAddressToAssign"); ?>',
					type: 'post', 
					data: {
						'ip': ipToFind
					},
						...
						if (parseInt(response.found) == 0) {
							...
							if (response.used == 'Y') {
								...
								url = url + '/' + username + '/' + realm + '/' + 
									response.location.replace(/ /g, '~') + '/' + response.gpon + '/' + response.cabinetForDropdown + '/' + ipToFind + '/-';
								top.location.href = url;
							}
						}
					},
				...
				$.ajax({
					...
					success: function (resp) {
						...
						if (parseInt(response.found) == 0) {
							...
							url = url + '/' + username + '/' + realm + '/' + 
								response.location.replace(/ /g, '~') + '/' + gpon + '/' + response.cabinetForDropdown.replace(/ /g, '~') + '/-/' + cabinetToFind.replace(/ /g, '~');
							top.location.href = url;
						}
					},
					error: function (resp) {
