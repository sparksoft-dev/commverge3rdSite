...
<body style="padding-right:20px; padding-bottom:20px">
	<div align="left">
		<h3 class="style1">IP Addresses: Bulk Unassign</h3>
	</div>
	...
	<div>
		...
		?>
		<form action="<?php echo base_url('main/bulkIPUnassignProcess'); ?>" method="POST" enctype="multipart/form-data">
			<table>
				<tr>
		...
		?>
		<form action="<?php echo base_url('main/bulkIPUnassignProcess'); ?>" method="POST">
		...
		<?php
				...
				if (count($notProcessedIPUnused) != 0) {
		?>
		<br /><br /><br />
		<span class="errorMsg">The following records were not processed because the IP addresses are already unused.</span>
		<table cellspacing="1" cellpadding="3" style="background-color:#cccccc;" border="0" width="100%">
			<tbody>
				<tr>
					<td class="smallFontGRYBG" align="left" nowrap>&nbsp;</td>
					...
					<?php
					for ($i = 0; $i < count($notProcessedIPUnused); $i++) {
						$row = $notProcessedIPUnused[$i];
					?>
				<tr>
					<td class="smallFontWHTBG" nowrap><?php echo ($i + 1); ?></td>
					<td class="smallFontWHTBG" nowrap><?php echo $row[0]; ?></td>
