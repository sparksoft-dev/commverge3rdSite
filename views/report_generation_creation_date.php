...
<body style="padding-right:20px; padding-bottom:20px;">
	<div align="left">
		<h3 class="style1">Report Generation: Creation Date</h3>
	</div>
	...
	<div align="left">
		...
		<form name="frmMain" action="<?php echo base_url('reports/generateSubscriberCreationReport'); ?>" method="post">
							...
							<table>
								...
								<tr>
									...
									<td class="xsmallFontWHTBG">
										...
										$end_month = is_null($dateend) ? date('m') : $dateendParts[1];
										$end_day = is_null($dateend) ? date('d') : $dateendParts[2];
										$end_year = is_null($dateend) ? date('Y') : $dateendParts[0];
										?>
		...
		<table width="100%">
			<tr>
				<td colspan="10">
					...
					if ($pageStart != 0) {
						$j = $pageStart - 1;
						echo '<a href="'.base_url("reports/generateSubscriberCreationReport/1/count/".
							$start_year."-".$start_month."-".$start_day."/".$end_year."-".$end_month."-".$end_day."/".($j * $max)."/".$max).'">&lt;&lt;</a>';
						echo ' |';
					}
					for ($j = $pageStart; $j < $pageEnd; $j++) {
						echo '<a href="'.base_url("reports/generateSubscriberCreationReport/1/count/".
							$start_year."-".$start_month."-".$start_day."/".$end_year."-".$end_month."-".$end_day."/".($j * $max)."/".$max).'">'.($j + 1).'</a>';
						echo ' | ';
					}
					if ($pageEnd < $pages) {
						$j = $pageEnd;
	...
	<script type="text/javascript">
		...
		$('select[name="max"]').on('change', function (event) {
			 var _this = $(this),
				max = _this.val(),
				location = '<?php echo base_url("reports/generateSubscriberCreationReport/1/count/".
					$start_year."-".$start_month."-".$start_day."/".$end_year."-".$end_month."-".$end_day."/0"); ?>';
		...
		$(document).ready(function () {
			$('#extractBtn').on('click', function (evt) {
				...
				$.ajax({
					url: '<?php echo base_url("reports/ajaxGenerateSubscriberCreationReport"); ?>',
					type: 'POST',
					data: {
